...
class utilitySuite:
    def __init__(self, config) -> None:
        ...
        self.logger = Logger(config.save_dir, config.exp_name)
        self.rec = ListDict()
...
class pltUtils:
    ...
    def get_fig(self, grid=[1, 1], figsize=[8, 6], dpi=150, gridline=False):
...
class open3dUtils:
    ...
    def draw_point_cloud(self, points, colors=None):
        pcd = self.o3d.geometry.PointCloud()
        pcd.points = self.o3d.utility.Vector3dVector(points)
        if colors is not None:
            if len(colors.shape) == 1:
                colors = np.vstack([colors, colors, colors]).T
            pcd.colors = self.o3d.utility.Vector3dVector(colors)
        self.o3d.visualization.draw_geometries([pcd])
...
def readCSV(filename, delimiter=None, skiprows=0):
    import csv
    lines = []
